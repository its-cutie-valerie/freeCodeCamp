...
```js
assert.exists(document.querySelector('ul > li:nth-child(3) button'));
```

Your `button` element should have an `aria-expanded` attribute set to `false`.

```js
assert.equal(document.querySelector('ul > li:nth-child(3) button')?.getAttribute('aria-expanded'),'false');
```

Your `button` element should have the text `Apps`.

```js
assert.equal(document.querySelector('ul > li:nth-child(3) button')?.textContent,'Apps');
```
